    ...
    <table class="table" id="table2excel" border="2">
        ...
        <tr>
           ...
           // echo '<th>T</th>';
			...
			if($engpro){
			$EmpModel = EmpDetails::find()->where(['id'=>$engpro->emp_id])->andwhere(['!=','category','HO Staff'])->andwhere(['!=','category','BO Staff'])->all();
				}else{
				$EmpModel = EmpDetails::find()->where(['!=','category','HO Staff'])->andwhere(['!=','category','BO Staff'])->all();
				}
