...
class PostrectrainingController extends Controller
{
	...
	public function actionAjaxTrainingselection(){
		...
		echo "<table class='table'>  <th>Name</th> <th><input type='checkbox' name='selection_all' class='select-on-check-all' id='selectall' value='1' ></th> ";
			$lp	=	0;
			if($trainigqry[0]->training_type == 'new'){
			...
			if($trainigqry[0]->training_type == 'existing'){
				foreach($trainigqry as $restraining){
	...
	public function actionAjaxTrainingselectionreport(){
		...
		echo '<table class="table-bordered" style="font-size: 12px;"> <thead> <th>Name</th> ';
			foreach($trainingtopics as $restopics){
				$trg_attqry1 = PorecTrainingAttendance::find()->where(['training_batch_id'=>$data['training_batch_id'],'topic_id'=>$restopics->id])->one();
				if($restopics->id	==	$trg_attqry1['topic_id']){
					$attendance_date	=	date('d-m-Y',strtotime($trg_attqry1['attendance_date']));
				}else{
					$attendance_date	=	'';
				}
				echo "<th>".$restopics->topic_name."</br>".$attendance_date."</th>";
			}
	...
	public function actionAjaxExistingtrainingselectionreport(){
		...
		echo '<table class="table-bordered" style="font-size: 12px;"> <thead> <th>Name</th> ';
			foreach($trainingtopics as $restopics){				
				$trg_attqry1 = PorecTrainingAttendanceExisting::find()->where(['training_batch_id'=>$data['training_batch_id'],'topic_id'=>$restopics->id])->one();
				if($restopics->id	==	$trg_attqry1['topic_id']){
					$attendance_date	=	date('d-m-Y',strtotime($trg_attqry1['attendance_date']));
				}else{
					$attendance_date	=	'';
    ...
    {  
		...
		if ($data) {			
			...
			$model->rec_id				=	$data['selection'];
			$model->porec_id			=	$data['porec_id'];
			$model->print_date			=	$data['PorecTraining']['print_date'];
			$model->faculty_name		=	$data['TrainingFaculty']['faculty_name'];
			$model->training_topic_id	=	$data['TrainingTopics']['topic_name'];
