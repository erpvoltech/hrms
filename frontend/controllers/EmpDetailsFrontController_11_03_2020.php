   ...
   public function actionExportwodata() {
	...
	public function actionLogin() {
		...
		if ($model->load(Yii::$app->request->post())) {			
			#exit;
			$modelEmp = EmpDetailsFront::find()->where(['empcode' => $model->empcode])->one();
			...
			if($model->emp_password == $modelEmp->emp_password && $modelEmp->consolidated_status == 'No' ){
				...
				if($modelEmp->consolidated_status == 'Yes'){
					Yii::$app->session->setFlash('error', 'You are completed the updation!!!.');
				}
   ...
   public function actionUpdate($id) {
		...
		if($model->consolidated_status == 'No'){	  
		  ...
		  if ($model->load($post)) {
			 ...
			 return $this->redirect(['education-details', 'id' => $model->id]);
		  } else {
			 return $this->render('employee_form', [
						 'model' => $model,
			 ]);
