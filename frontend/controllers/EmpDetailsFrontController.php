   ...
   public function actionCreate($ecode) {
	...
	public function actionVerifyecode(){
		...
		foreach($selection as $res){		
			...
			$empedubackmodel2							=	EmpFamilydetails::findOne($empfamilybackmodel->id);
			
			$modelfamilyRef = new EmpFamilydetails();
			$oldfamIds = EmpFamilydetails::find()->select('id')->where(['empid' => $modelbackemp->id])->asArray()->all();
			$oldfamIds = ArrayHelper::getColumn($oldfamIds, 'id');
			
			$modelfamilyReffront = new EmpFamilydetailsFront();
			$oldfamIdsfront = EmpFamilydetailsFront::find()->select('id')->where(['empid' => $model->id])->asArray()->all();
			$oldfamIdsfront = ArrayHelper::getColumn($oldfamIdsfront, 'id');
			
			$modelfamback = EmpFamilydetails::findAll(['id' => $oldfamIds]);
			$modelfamback = (empty($modelfamback)) ? [new EmpFamilydetails] : $modelfamback;
			
			$modelfamfront = EmpFamilydetailsFront::findAll(['id' => $oldfamIdsfront]);
			$modelfamfront = (empty($modelfamfront)) ? [new EmpFamilydetailsFront] : $modelfamfront;
			
			$modelfamily = Model::createMultiple(EmpFamilydetails::classname(), $modelfamback);
			$modelfamilyfront = Model::createMultiple(EmpFamilydetailsFront::classname(), $modelfamfront);
				#if (Model::loadMultiple($modelEducation, Yii::$app->request->post())) {
				#echo "<pre>";print_r($modelfamfront);echo "</pre>";
				#exit;
				 $newFamIds = ArrayHelper::getColumn($modelfamily, 'id');

				 $delFamIds = array_diff($oldfamIds, $newFamIds);
				 if (!empty($delFamIds))
					EmpFamilydetails::deleteAll(['id' => $delFamIds]);
				#echo "<pre>";print_r($modelfamilyfront);echo "</pre>";
				#exit;

				 $transaction = \Yii::$app->db->beginTransaction();
				 try {
					foreach ($modelfamfront as $modelfamres) { /* save Education */
						$modelfamres->empid = $modelbackemp->id;

						$modelfambc = new EmpFamilydetails();
						$modelfamFrontres2						=	EmpFamilydetailsFront::findOne($modelfamres->id);
						
						$modelfambc->attributes				=	$modelfamFrontres2->attributes;						
						$modelfambc->empid = $modelbackemp->id;
						
						#echo "<pre>";print_r($modelfambc);echo "</pre>";
						#exit;

					   if (!($flag = $modelfambc->save(false))) {
			...
			$oldEduIds = EmpEducationdetails::find()->select('id')->where(['empid' => $modelbackemp->id])->asArray()->all();
			$oldEduIds = ArrayHelper::getColumn($oldEduIds, 'id');
			
			$modelEduFrontRef = new EmpEducationdetailsFront();
			$oldEduIdsFront = EmpEducationdetailsFront::find()->select('id')->where(['empid' => $model->id])->asArray()->all();
			$oldEduIdsFront = ArrayHelper::getColumn($oldEduIdsFront, 'id');

			  $modeleduback = EmpEducationdetails::findAll(['id' => $oldEduIds]);
			  $modeleduback = (empty($modeleduback)) ? [new EmpEducationdetails] : $modeleduback;
			  
			  $modeledufront = EmpEducationdetailsFront::findAll(['id' => $oldEduIdsFront]);
			  $modeledufront = (empty($modeledufront)) ? [new EmpEducationdetailsFront] : $modeledufront;
			
			  $modelEducation = Model::createMultiple(EmpEducationdetails::classname(), $modeleduback);
			  $modelEducationFront = Model::createMultiple(EmpEducationdetailsFront::classname(), $modeledufront);
			  #if (Model::loadMultiple($modelEducation, Yii::$app->request->post())) {
				 ...
				 $transaction = \Yii::$app->db->beginTransaction();
				 try {
					foreach ($modeledufront as $modelEdu) { /* save Education */
						$modelEdu->empid = $modelbackemp->id;

						$modeledubc = new EmpEducationdetails();
						$modeleduFrontres2						=	EmpEducationdetailsFront::findOne($modelEdu->id);
						
						$modeledubc->attributes				=	$modeleduFrontres2->attributes;						
						$modeledubc->empid = $modelbackemp->id;
						
					   if (!($flag = $modeledubc->save(false))) {
						  $transaction->rollBack();
						  break;
					   }
					}
			...
			$empbankbackmodel2						=	EmpBankdetails::findOne($empbankbackmodel->id);
			  ...
			  #echo "<pre>";print_r($modelFront);echo "</pre>";
			  #exit;
			  #if (Model::loadMultiple($modelBankfront)) {
				  #echo "hi";
				  #exit;
				 ...
				 $transaction = \Yii::$app->db->beginTransaction();
				 try {
					foreach ($modelFront as $modelFrontres) { /* save Certificate */
					
					#echo "<pre>";print_r($modelFrontres);echo "</pre>";
					#exit;
					
						
						$modelBnk = new EmpBankdetails();
						$modelFrontres2						=	EmpBankdetailsFront::findOne($modelFrontres->id);
						
						$modelBnk->attributes				=	$modelFrontres2->attributes;						
						$modelBnk->empid = $modelbackemp->id;
						#echo "<pre>";print_r($modelBnk);echo "</pre>";
						#exit;
			...
			$oldemployementIds = PreviousEmployment::find()->select('id')->where(['empid' => $modelbackemp->id])->asArray()->all();
			$oldemployementIds = ArrayHelper::getColumn($oldemployementIds, 'id');
			
			$modelEmploymentReffront = new PreviousEmploymentFront();
			$oldemployementfrontIds = PreviousEmploymentFront::find()->select('id')->where(['empid' => $model->id])->asArray()->all();
			$oldemployementfrontIds = ArrayHelper::getColumn($oldemployementfrontIds, 'id');

			  $modelbackprevemp = PreviousEmployment::findAll(['id' => $oldemployementIds]);
			  $modelbackprevemp = (empty($modelbackprevemp)) ? [new PreviousEmployment] : $modelbackprevemp;

			  $modelfrontemp = PreviousEmploymentFront::findAll(['id' => $oldemployementfrontIds]);
			  $modelfrontemp = (empty($modelfrontemp)) ? [new PreviousEmploymentFront] : $modelfrontemp;
 
			  
			  $modelEmployment = Model::createMultiple(PreviousEmploymentFront::classname(), $modelfrontemp);
			  #if (Model::loadMultiple($modelEmployment, Yii::$app->request->post())) {
				  
				  #echo "<pre>";print_r($modelfrontemp);echo "</pre>";
				  #exit;
				 ...
				 $transaction = \Yii::$app->db->beginTransaction();
				 try {
					foreach ($modelfrontemp as $Employment) { /* save Certificate */
					   $Employment->empid = $model->id;
					  ...
					  if(!empty($Employment->work_to) && $Employment->work_to !='1970-01-01' && $Employment->work_to != '01-01-1970'){
						...
						$modelprevemp = new PreviousEmployment();
						$modelFrontprevemp2						=	PreviousEmploymentFront::findOne($Employment->id);
						
						$modelprevemp->attributes				=	$modelFrontprevemp2->attributes;						
						$modelprevemp->empid = $modelbackemp->id; 

						#echo "<pre>";print_r($modelprevemp);echo "</pre>";
						#exit;
						
					  if (!($flag = $modelprevemp->save())) {
						  $transaction->rollBack();
						  break;
					   }
					}
