...
$objPHPExcel->getActiveSheet()
        ...
        ->setCellValue('N1', 'User Division')
        ->setCellValue('O1', 'Insured To')
        ->setCellValue('P1', 'Equipment Service')
...
$query = 'SELECT * FROM vg_insurance_equipment';
if (count($conditions) > 0) {
    $query .= " WHERE " . implode(' AND ', $conditions);
    $model = VgInsuranceEquipment::findBySql($query)->all();
} else {
    $model = VgInsuranceEquipment::find()->all();
}
...
header('Content-Disposition: attachment;filename="Equipment Policy.xlsx"');
