...
error_reporting(0);
		...
		foreach ($modelUnit as $unit){
			...
			foreach ($UnitGroupModel as $groupmodel){
				if($data = unserialize($_GET['group'])){
				 ...
				 $modelCount = EmpSalary::find()->joinWith(['employee','bank'])->where(['month'=>$month,'emp_salary.unit_id'=>$groupmodel->unit_id,'emp_salary.division_id'=>$groupmodel->division_id])
				 ->andWhere(['NOT IN', 'emp_details.status', ['Non-paid Leave']])
				 ->andWhere(['IN','emp_bankdetails.bankname',['Bank of Baroda','BOB','Baroda','Baroda bank']])
		...
		$row =2;
			...
			->setCellValue('G1', 'Net Amount') 
			->setCellValue('H1', 'Unit') 
			->setCellValue('I1', 'Division') 
			->setCellValue('J1', 'Customer') 
		...
		foreach ($modelUnit as $unit){	
			...
			foreach ($UnitGroupModel as $groupmodel){
				...
				//->andWhere(['IS','emp_bankdetails.bankname',NULL])
				 ...
				 $modelSal = EmpSalary::find()->joinWith(['employee','bank'])->where(['month'=>$month,'emp_salary.unit_id'=>$groupmodel->unit_id,'emp_salary.division_id'=>$groupmodel->division_id])				
				 ->andWhere(['NOT IN','emp_bankdetails.bankname',['Bank of Baroda','BOB','Baroda','Baroda bank','State Bank of India','SBI']])
				// ->andWhere(['IS','emp_bankdetails.bankname',NULL])
				 ->andWhere(['NOT IN', 'emp_details.status', ['Non-paid Leave']])
				 ->orderBy(['net_amount' => SORT_DESC])->all();	
				...
				$objPHPExcel->getActiveSheet()->getStyle('E2:E'.($row-1))->getNumberFormat()->setFormatCode(PHPExcel_Style_NumberFormat::FORMAT_TEXT);
			  
		       $objPHPExcel->getActiveSheet()->mergeCells('A'.$row.':F'.$row);
			   $objPHPExcel->getActiveSheet()->mergeCells('H'.$row.':K'.$row);	
			   $objPHPExcel->getActiveSheet()->setCellValue('A'.$row, 'Grand Total')
										     ->setCellValue('G'.$row, '=SUM(G1:G'.($row-1).')');
			   $objPHPExcel->getActiveSheet()->setSharedStyle($sharedStyle4, "A".$row.":K".$row);							 
