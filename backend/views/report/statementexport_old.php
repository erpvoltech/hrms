...
use common\models\UnitGroup;
	   ...
	   if (Yii::$app->getRequest()->getQueryParam('month'))
		...
		$grand_ctc = 0;
			...
			->setCellValue('AF1', 'Mobile')
			->setCellValue('AG1', 'Loan')
			->setCellValue('AH1', 'Rent')
			->setCellValue('AI1', 'TDS')
			->setCellValue('AJ1', 'LWF')
			->setCellValue('AK1', 'Advance')
			->setCellValue('AL1', 'Other Deduction')
			->setCellValue('AM1', 'Total Deduction')
			->setCellValue('AN1', 'Net Amount')
			->setCellValue('AO1', 'PF Employer Contribution')
			->setCellValue('AP1', 'ESI Employer Contribution')		
			->setCellValue('AQ1', 'LTA Employer Contribution')
			->setCellValue('AR1', 'MED Employer Contribution')
			->setCellValue('AS1', 'PLI Employer Contribution')
			->setCellValue('AT1', 'Earned CTC')
			->setCellValue('AU1', 'Bank Transfer');
			...
			foreach ($UnitGroupModel as $groupmodel){
				...
				$modelSal = EmpSalary::find()->joinWith(['employee'])
				 ...
				 ->orderBy(['net_amount' => SORT_DESC])->all();
				  ...
				  $modelSalcount = EmpSalary::find()->where(['month'=>$month,'emp_salary.unit_id'=>$groupmodel->unit_id,'emp_salary.division_id'=>$groupmodel->division_id])
				 ->count();				 
				}
				if($modelSalcount > 0) {
				 ...
				 $objPHPExcel->getActiveSheet()->mergeCells('A'.$row.':AU'.$row);
				 $objPHPExcel->getActiveSheet()->getStyle('A2')->getFont()->setBold(true);
				 $objPHPExcel->getActiveSheet()->getStyle('A'.$row)->getFont()->setBold(true);
				 $objPHPExcel->getActiveSheet()->getStyle('A'.$row.':AU'.$row)->applyFromArray($styleArray);
				...
				$Remu = EmpRemunerationDetails::find()->where(['empid'=>$data->empid])->one();	
				 ...
				 $dataArray = array(
				  ...
				  $objPHPExcel->getActiveSheet()->setSharedStyle($sharedStyle2, 'A' . $fstratrow . ':AU' . $row);
				  $objPHPExcel->getActiveSheet()->mergeCells('A'.$row.':E'.$row);				  
				  $objPHPExcel->getActiveSheet()->getStyle('A'.$row.':AU'.$row)->getFont()->setBold(true);	
				  $objPHPExcel->getActiveSheet()->getStyle('A'.$fstratrow.':AU'.$row)->applyFromArray($styleArray);
				  $objPHPExcel->getActiveSheet()->getRowDimension('A'.$fstratrow.':AU'.$row)->setRowHeight(30);				 
			...
			/*	$objPHPExcel->getActiveSheet()->setCellValue('A'.$row, 'Grand Total');
			   ...
			   $objPHPExcel->getActiveSheet()->setSharedStyle($sharedStyle1, "A1:AU1");
			   $objPHPExcel->getActiveSheet()->getStyle('A1:AU1')->getFont()->setBold(true);
			   $objPHPExcel->getActiveSheet()->getStyle('A1:AU1')->applyFromArray($styleArray);
			   $objPHPExcel->getActiveSheet()->getRowDimension('A1:AU1')->setRowHeight(30);
