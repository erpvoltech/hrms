...
if (Yii::$app->getRequest()->getQueryParam('unit')) {
   ...
   echo $this->render('statement', ['model' => $model, 'group' => $group,'month'=>$month,'unit' => $unit]);
   ?>
...
<th>TDS</th><th>LWF</th><th>Other Deduction</th><th>Total Deduction</th><th>Net Amount</th></tr>';
	...
	exit;
      ...
      $division = Division::find()->Where(['id'=>$groupmodel->division_id])->One();
		...
		//$modelSal = EmpSalary::find()->where(['month'=>$month,'unit_id'=>$unit->id,'division_id'=>$division->id])->orderBy(['net_amount' => SORT_DESC])->all();
			if($modelSal) {
