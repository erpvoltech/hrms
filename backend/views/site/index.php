                      ...
                      <div class="ch-item ch-img-2">
							...
							<?php //$salmonth = SalaryMonth::find()->orderBy(['id' => SORT_DESC])->one();
								...
								<a href="emp-details/newjoin-index"><h3> New Joining <?= EmpDetails::find()
									->where(['between', 'doj', $fromdate , $currentdate])->count() ?></h3> </a>
							...
							<?php //$salmonth = SalaryMonth::find()->orderBy(['id' => SORT_DESC])->one();
								...
								?>
								 <a href="emp-details/resigned-index" ><h3>  Resigned <br> <?= EmpDetails::find()
									->where(['between', 'last_working_date', $fromdate , $currentdate])
               ...
               <div class="box-body">                  
                  <div class="info-box">
                     ...
                     <div class="info-box-content">                       
						...
						$upcommingbday = EmpDetails::find()->joinWith(['employeePersonalDetail'])
						->where('DAY(emp_personaldetails.birthday) = DAY( CURDATE() +1 ) AND MONTH(emp_personaldetails.birthday)=MONTH(CURDATE())')											
						->andWhere(['in', 'emp_details.status', ['Active','Notice Period']])
						->orderBy(['DAY(emp_personaldetails.birthday)'=>SORT_ASC])->all();
                        ...
                        //$birthday = EmpPersonaldetails::find()->where('DAY(birthday) = DAY(CURDATE()) AND MONTH(birthday)=MONTH(CURDATE())')->all();
						 echo '<table>';
                                echo '<tr><th>Name</th><th>Date</th><th>Designation</th><th>Unit</th><th>Department</th></tr>';
                                echo '<tr><td colspan=5 align=center><u>Today`s Birthday</u></td></tr>';
                        foreach ($birthday as $bday) {					
						echo '<div class="col-md-10">';
						
						  echo '<tr><td>'. $bday->empname . '</td><td>'.Yii::$app->formatter->asDate($bday->employeePersonalDetail->birthday, 'php:d-m-'.$current) . '</td><td>'. $bday->designation->designation . '</td><td>' . $bday->units->name.'</td><td>' . $bday->department->name.'</td></tr>';
						echo '</div>';						
                        }
                        echo '<br>';
                       		 ########### next 2 days bday #########
                        echo '<tr><td colspan=5 align=center><u>Upcomming Birthday</u></td></tr>';
                        foreach ($upcommingbday as $upbday) {					
						echo '<div class="col-md-10">';
						
						  echo '<tr><td>'. $upbday->empname . '</td><td>'.Yii::$app->formatter->asDate($upbday->employeePersonalDetail->birthday, 'php:d-m-'.$current).'</td><td>'. $upbday->designation->designation . '</td><td>' . $upbday->units->name.'</td><td>' . $upbday->department->name.'</td></tr>';
						echo '</div>';						
                        }
						 echo '</table>';
						echo '</div>';
...
$vehicleReminderpoll = VgInsuranceVehicle::find()->where('pollution_valid_to BETWEEN NOW() AND DATE_ADD(NOW(), INTERVAL 30 DAY)')->orderBy(['pollution_valid_to' => SORT_ASC])->all();

//$vehicleExpired = VgInsuranceVehicle::find()->where('valid_to < DATE(NOW())')->all();

//$current = date('Y-m-d', strtotime('-1 month'));

echo'Curr'.$current = date('Y-m-d');
echo 'Pre'.$prev = date('Y-m-01', strtotime('-1 month'));

/*$vehicleExpired = VgInsuranceVehicle::find()->where(['between', 'valid_to', $prev, $current])
					 ->andWhere(['!=', 'financial_year', '2019-2021'])
					->all();*/

$vehicleExpired = VgInsuranceVehicle::find()
					->where(['=', 'financial_year', '2021-2022'])
...
echo '<div class="modal-content">
    <table style="font-size:11px;" >
    <caption style="text-align: center; font-weight: bold;">POLLUTION</caption>
                        <tr>
                            <th>Policy No</th>
                            <th>Vehicle Name</th>
                            <th>Reg.No</th>
                            <th>Pollution Valid From</th>
                            <th>Pollution Valid To</th>
                            <th>Sum Insured</th>
                            <th>Premium Paid</th>
                            <th>Insured To</th>
                            <th>Location</th>
...
foreach ($vehicleReminderpoll as $data) {
    ?>
    <tr>
        <td> <?= $data->insurance_no ?> </td>
        <td><?= $data->property_name ?></td>
        <td><?= $data->property_no ?></td>
        <td> <?= date('d.m.Y', strtotime($data->pollution_valid_from)) ?> </td>
        <td style="color: red;font-weight: bold;"> <?= date('d.m.Y', strtotime($data->pollution_valid_to)) ?> </td>
        <td style="text-align: right;"><?= $data->sum_insured ?></td>
        <td style="text-align: right;"><?= $data->premium_paid ?></td>
        <td><?= $data->insured_to ?></td>
        <td><?= $data->location ?></td>
