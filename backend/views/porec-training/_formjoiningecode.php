...
<h3 class="panel-title">Assign Ecode</h3>
</div>
<?php $form = ActiveForm::begin(			
    ['action' => ['porec-training/assignecode'],'layout' => 'horizontal']);  
	?>	
		</br></br>		
		<table align="center"> 
		<thead>
		...
		<?php
		$queryrec = Recruitment::find()->where(['join_status'=>'joined'])->all();
		foreach($queryrec as $res){
		?>			
			<tr>
				<td>
					<input type="hidden" name="recid[]" id="recid_<?php echo $res->id; ?>" class="recid" value="<?php echo $res->id; ?>" >
					<?php if($res->ecode == ''){ ?>
					<input type="text" name="ecode[]" id="ecode_<?php echo $res->id; ?>" class="ecode" value="" placeholder="Ecode" >
					<?php } else{ ?>
					<input type="text" name="ecode[]" id="ecode_<?php echo $res->id; ?>" class="ecode" value="<?php echo $res->ecode; ?>" >
					<?php } ?>
				</td>
				<td><?php echo $res->name; ?> </td>
				<td><?php echo $res->register_no; ?></td>
				<td><?php echo $res->join_status; ?></td>
			</tr>		
		<?php
		}		
		?>
	...
	<div class="row" align="center">		
		<div class="col-md-12" >
			<input type="submit" name="ecode_join" id="ecode_join" class="btn-sm btn-success" value="Submit" >
		</div>	
	</div>
	</br>
    <?php ActiveForm::end();  ?>
