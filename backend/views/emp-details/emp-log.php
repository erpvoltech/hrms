...
error_reporting(0);
/*$salfirst= EmpSalary::find()->where(['empid'=>$model->id])->orderBy('date')->one();
$Emp = EmpDetails::find()->where(['id'=>$model->id])->one();
$salmodel= EmpSalary::find()->where(['empid'=>$model->id])->orderBy('date')->all();
$promotionmodel = EmpPromotion::find()->where(['empid'=>$model->id])->orderBy('effectdate')->all();
 $firstunit = $salfirst->unit_id;
 $firstdiv = $salfirst->division_id;*/
...
$salmodel= EngineerTransfer::find()->where(['empid'=>$model->id])->orderBy('transfer_date')->all();
$selunit = EngineertransferProject::find()->where(['empid'=>$model->id])->orderBy('transfer_date')->all();
$promotionmodel = EmpPromotion::find()->where(['empid'=>$model->id])->orderBy('effectdate')->all();
