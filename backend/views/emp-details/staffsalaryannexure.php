...
$benefits = EmpBenefits::find()->where(['wl' =>$model->work_level])->one();	
	...
	if($model->work_level == 'WL5' || $model->work_level == 'WL6'){		
		$benefits_gpa = EmpGpaBenifits::find()->where(['wl' => $worklevel])->one();		
	} else {
		$benefits_gpa = EmpGpaBenifits::find()->where(['wl' => $worklevel,'grade'=>$model->grade])->one();
	}
  ...
  $gmc_fellow_share = VgGmcHierarchy::find()->where(['sum_insured' =>$statutory->gmc_sum_insured,'age_group'=>$statutory->age_group])->one();
	...
	$subtotalc =  round($provident_fund_er + $model->employer_esi_contribution + $pliAmt + $variable_allowance + round($statutory->gpa_premium/12) + round($statutory->gmc_premium/24));
...
<tr><td ><b> Net Salary (A-B) </b></td><td style="text-align: right; font-weight: bold;"><?= number_format(($totalactual) - ($subtotalb),2,'.', '')?></td><td style="text-align: right; font-weight: bold;"><?= number_format(($totalactual * 12) - ($subtotalb * 12),2,'.', '') ?></td></tr>
...
<tr><td align="center"><b>CTC (A+C) </b></td><td style="text-align: right; font-weight: bold;"> <?= number_format($totalactual + $subtotalc,2, '.','')?></td><td style="text-align: right; font-weight: bold;"> <?= number_format(($totalactual + $subtotalc) * 12,2,'.', '') ?></td><td></td></tr>


<tr><td colspan="4" align="center" style="font-weight: bold;"><b>Other Benefits / Eligibility</b> </td></tr>

<tr><td> Travel mode Eligibility </td><td colspan="2" style="text-align:center"><?=$benefits->travelmode_ts?></td> <td align="center" rowspan="5">The Eligibility values depends on work level, which is subjected to change by Management Discretion. Insurance Premium value shall differ based upon the offer by Insurer.</td></tr>
...
<tr><td colspan="2" style="border-right: 0px solid #ccc;border-bottom: 0px solid #ccc">For VOLTECH Engineers Private Limited,<br></td> <td colspan="2" style="border-left: 0px solid #ccc;border-bottom: 0px solid #ccc"> &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;  Received and Accepted</td></tr>

<tr><td colspan="4" style="border-top: 0px solid #ccc;border-bottom: 0px solid #ccc"> <?php /*Html::img("@web/img/signature.png")*/?> <br> <br> <br>
</td></tr>
<tr><td colspan="2" style="border-right: 0px solid #ccc;border-top: 0px solid #ccc;border-bottom: 0px solid #ccc;"><b>M.UMAPATHI </b></td> <td colspan="2" style="border-left: 0px solid #ccc;border-top: 0px solid #ccc;border-bottom: 0px solid #ccc"> &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;  <?=$Emp->empname?><br></td></tr>
<tr><td colspan="2" style="border-right: 0px solid #ccc;border-top: 0px solid #ccc;"><b>Managing Director</b></td> <td colspan="2" style="border-left: 0px solid #ccc;border-top: 0px solid #ccc;"> &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;  Date:<br></td></tr>
