...
/*
	...
	if($statutoryModel->gpa_applicability == 'Yes') {	
		$gpa_amt = round($statutoryModel->gpa_premium / 12);
	} else {
	   $gpa_amt = 0;
	}
		
	if($statutoryModel->gmc_applicability == 'Yes') {	
		$gmc_amt = round($statutoryModel->gmc_premium / 24);
	} else {
	   $gmc_amt = 0;
	}	
	echo $gmc_amt;
    ...
    <div class="panel panel-info">
        <div class="panel-heading">          
                <div class="pull-right" style="padding-right:10px"> 
				        <a href="print-empdetails?id=<?=$model->id?>" title="Print" target="_blank"><i class='glyphicon glyphicon-print' style="padding-right:10px"></i></a>
...
<div class="col-md-2">	
 ...
 <?php
 if($orderempid) {   	 
	  echo '<a href="order-update?id='.$model->id.'" class="list-group-item">Appointment Generate</a>';
	 } else {
	  echo '<a href="order-create?id='.$model->id.'" class="list-group-item">Appointment Generate</a>';	 
	 }
 ?> 
  <a href="appointmentpdf?id=<?=$model->id?>" class="list-group-item">Appointment PDF</a>
  <a href="staffsalaryannexure?id=<?=$model->id?>" class="list-group-item">Annexure</a>
  <a href="<?= \yii::$app->homeUrl ?>/document/index?type=1&id=<?=$model->id?>" class="list-group-item">Bonafide</a>
  <a href="<?= \yii::$app->homeUrl ?>/document/index?type=2&id=<?=$model->id?>" class="list-group-item">Relieving Letter</a>
  <a href="<?= \yii::$app->homeUrl ?>/document/index?type=3&id=<?=$model->id?>" class="list-group-item">Show Cause Notice</a>
  <a href="emp-log?id=<?=$model->id?>" class="list-group-item" target="_blank">Employee Tracking</a>
</div>
</div>
</div>
