...
<div class="row">
  ...
  <button class="btn btn-default " type="button" id="exportemp" data-toggle="dropdown" aria-haspopup="true" aria-expanded="true">
   Permanent Emp. Details </button> <br>
	<button class="btn btn-default " type="button" id="contractexport" data-toggle="dropdown" aria-haspopup="true" aria-expanded="true">
    Contract Emp. Details </button>
...
<?php
           $script = <<<JS
			 ...
			 if(exporttype == 3){
			 var printWindow = window.open('promotion-export?unit='+unit+'&design='+design+'&dept='+dept+'&division='+div+'&categ='+categ+'&type='+exporttype, 'Print', 'left=200, top=200, width=500, height=150, toolbar=0, resizable=0');
			...
			$("#contractexport").click(function() {
			 ...
			 var exporttype = $('input[name=datavalue]:checked').val();
			   if(exporttype == 3){
			 var printWindow = window.open('promotion-exportcontract?unit='+unit+'&design='+design+'&dept='+dept+'&division='+div+'&categ='+categ+'&type='+exporttype, 'Print', 'left=200, top=200, width=500, height=150, toolbar=0, resizable=0');
				printWindow.document.title = "Downloading";
				printWindow.addEventListener('load', function () {
				}, true);
			 }else if(exporttype) {
				var printWindow = window.open('export-contractemp?unit='+unit+'&design='+design+'&dept='+dept+'&division='+div+'&categ='+categ+'&type='+exporttype, 'Print', 'left=200, top=200, width=500, height=150, toolbar=0, resizable=0');
				printWindow.document.title = "Downloading";
								printWindow.addEventListener('load', function () {
								}, true);
				 } else {
