...
$objPHPExcel->getActiveSheet()
        ...
        ->setCellValue('AZ1', 'PAN No')
        ->setCellValue('BA1', 'Aadhaar No')
        ->setCellValue('BB1', 'Passport No')
        ->setCellValue('BC1', 'Passport Valid')
        ->setCellValue('BD1', 'Passport Remarks')
        ->setCellValue('BE1', 'Voter ID')
        ->setCellValue('BF1', 'Driving Licence No')
        ->setCellValue('BG1', 'Licence Categories')
        ->setCellValue('BH1', 'Licence Remarks')
        ->setCellValue('BI1', 'Referred By')
        ->setCellValue('BJ1', 'Probation')
        ->setCellValue('BK1', 'Appraisal Month')
        ->setCellValue('BL1', 'Latest Promotion Date')
        ->setCellValue('BM1', 'Joining Status')
        ->setCellValue('BN1', 'Previous Experience')
		->setCellValue('BO1', 'Last Working Date')
		->setCellValue('BP1', 'Resignation Date')
        ->setCellValue('BQ1', 'Date of Leaving')
        ->setCellValue('BR1', 'Reason for Leaving')
        ->setCellValue('BS1', 'Status')		
        ->setCellValue('BT1', 'Basic')
        ->setCellValue('BU1', 'HRA')
        ->setCellValue('BV1', 'DA')
        ->setCellValue('BW1', 'Personpay')
        ->setCellValue('BX1', 'Dust Allowance')
        ->setCellValue('BY1', 'Guaranteed Benefit')
        ->setCellValue('BZ1', 'Other Allowance')
		->setCellValue('CA1', 'Washing Allowance')
		->setCellValue('CB1', 'Miscellaneous')
		->setCellValue('CC1', 'Food Allowance');
...
foreach ($model as $data) {
 ...
 $edu = EmpEducationdetails::find()->where(['empid' => $data->id])->one();
   ...
   $dataArray = array(
       ...
       $data->status,
	   $data->remuneration->basic,
	   $data->remuneration->hra,
	   $data->remuneration->dearness_allowance,
	   $data->remuneration->personpay,
	   $data->remuneration->dust_allowance,
	   $data->remuneration->guaranteed_benefit,
	   $data->remuneration->other_allowance,
	   $data->remuneration->washing_allowance,
	   $data->remuneration->misc,
...
$objPHPExcel->getActiveSheet()->setSharedStyle($sharedStyle1, "A1:CC1");
$objPHPExcel->getActiveSheet()->getStyle('A1:CC1')->getAlignment()->setWrapText(TRUE);
$objPHPExcel->getActiveSheet()->getStyle('A1:CC1')->getFont()->setBold(true);
$objPHPExcel->getActiveSheet()->setAutoFilter('A1:CC1');
...
foreach ($model as $data) {
   ...
   $objValidation14 = $objPHPExcel->getActiveSheet()->getCell('BI' . $i)->getDataValidation();
   ...
   $objValidation15 = $objPHPExcel->getActiveSheet()->getCell('BQ' . $i)->getDataValidation();
   ...
   $objValidation18 = $objPHPExcel->getActiveSheet()->getCell('AQ' . $i)->getDataValidation();
   ...
   $objValidation19 = $objPHPExcel->getActiveSheet()->getCell('BE' . $i)->getDataValidation();
   ...
   $objValidation20 = $objPHPExcel->getActiveSheet()->getCell('BK' . $i)->getDataValidation();
   ...
   $objValidation121 = $objPHPExcel->getActiveSheet()->getCell('AT' . $i)->getDataValidation();
   ...
   $objValidation122 = $objPHPExcel->getActiveSheet()->getCell('CA' . $i)->getDataValidation();
   ...
   $objValidation22 = $objPHPExcel->getActiveSheet()->getCell('AW' . $i)->getDataValidation();
   ...
   $objValidation22->setFormula1('"0-35,36-45,46-50,51-55,56-60,61-65,66-70,76-80"'); 
...
foreach ($model as $data) {
   $objPHPExcel->getActiveSheet()->setSharedStyle($sharedStyle2, 'A' . $j++ . ':CC' . $k++);
}
$j--;
$k--;
