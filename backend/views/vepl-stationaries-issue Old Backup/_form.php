            ...
            <?php foreach ($modelsGrnItem as $index => $modelGrnItem): ?>
                <div class="item panel panel-default"><!-- widgetBody -->
                    ...
                    <div class="panel-body">
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <!--<= $form->field($modelGrnItem, "[{$index}]issue_item_id")->dropDownList($stationaryData, ['prompt' => 'Select...','onchange' => 'totales($(this))']) ?>-->
                                <?=
                                $form->field($modelGrnItem, "[{$index}]issue_item_id")->widget(Select2::classname(), [
                                    'data' => $stationaryData,
...
<script>
    function codeAddress() {
        ...
        if (iq != '') {
            for (i = 0; i <= indexid; i++) {
                var item = $('#veplstationariesissuesub-' + i + '-issue_item_id').val();
                $.ajax({
                    type: "POST",
...
<script>
    function totales(item) {
        ...
        var item = $('#veplstationariesissuesub-' + index + '-issue_item_id').val();
