...
class VgInsuranceMotherPolicyController extends Controller {
    ...
    public function actionPolicyhierarchy($id) {
        $insuranceHierarchy = VgInsuranceHierarchy::find()->where(['master_policy_id' => $id])->all();
        return $this->render('policyhierarchy', ['insuranceHierarchy' => $insuranceHierarchy,]);
    }
    ...
    public function actionCreateasset() {
        $assetPolicy = new VgInsuranceMotherPolicy;

        if ($assetPolicy->load(Yii::$app->request->post())) {
            $assetPolicy->from_date = Yii::$app->formatter->asDate($assetPolicy->from_date, "yyyy-MM-dd");
            $assetPolicy->to_date = Yii::$app->formatter->asDate($assetPolicy->to_date, "yyyy-MM-dd");
            $assetPolicy->policy_for_id = 3;
            $assetPolicy->save();
            return $this->redirect(['/vg-insurance-agents/index']);
        }
