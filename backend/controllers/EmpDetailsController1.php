			...
			'verbs' => [
				...
				'access' => [
					...
					'rules' => [
						...
						[
							'allow' => true,
							'actions' => ['enggsalaryannexure','staffsalaryannexure','export-contractemp','export-emp','mis-export','export-bank','export-edu','export-certificate','export-family','index','view','export',
							...
							'remuneration','promotion-exportcontract','print-empdetails','appointmentpdf',
							],
										  'allow' => true,
						...
						[
							...
							'previous_employment','bank-details','certificates-details','education-details','personal-details','salarystructure','worklevel','remuneration','exportwodata',
							],
										  'allow' => true,
										  'matchCallback' => function ($rule, $action) {
						...
						[
							'allow' => true,
							'actions' => ['export-emp','mis-export','export-contractemp','export-bank','export-edu','export-certificate','export-family','index','view','create','update','export','promotion-export','import-employee','appointment-letter','generateorder','statutory-details',
							'previous_employment','bank-details','certificates-details','education-details','personal-details','salarystructure','worklevel','remuneration','exportwodata',
							],
										  'allow' => true,
										  'matchCallback' => function ($rule, $action) {
				  ...
				  if ($excelrow['EPF Applicability'] == 'Yes') {				
						if ($excelrow['Restrict EPF'] == 'Yes') {
							...
							if($statutory_rate_pf <= 15000){
								$employer_pf = round(($statutory_rate_pf * ($pf_esi_rates->epf_ac_1_er / 100)) + ($statutory_rate_pf * ($pf_esi_rates->epf_ac_10_er / 100)) + ($statutory_rate_pf * ($pf_esi_rates->epf_ac_2_er / 100)));				
							} else {
			   ...
			   if (!empty($excelrow['Emp. Code'])) { 
				...
				$modelEmp->confirmation_date =NULL; 
                 ...
                 $modelPer->passport_remark = $excelrow['Passport Remarks'];
				  ...
				  $modelRemu->gross_salary = $modelRemu->basic + $modelRemu->hra + $modelRemu->dearness_allowance + $modelRemu->personpay + $modelRemu->dust_allowance + $modelRemu->guaranteed_benefit + $modelRemu->other_allowance + $modelRemu->misc;
				...
				$model->empid = $id;
				 ...
				 if (!empty($model->pli) && $model->pli != 'NA') {
                  ...
                  if ($model->esi_applicability == 'Yes' && $model->gross_salary < 21000) {
                     $model->employer_esi_contribution = ceil(($model->gross_salary - $model->misc) * ($pf_esi_rates->esi_er / 100));
                  } else {
				  ...
				  if ($model->restrict_pf == 'Yes') {
					if($model->gross_salary <= 15000){
						$statutory_rate_pf = $model->gross_salary - ($model->hra + $model->misc);
						 ...
						 $model->employer_pf_contribution = round(($statutory_rate_pf * ($pf_esi_rates->epf_ac_1_er / 100)) + ($statutory_rate_pf * ($pf_esi_rates->epf_ac_10_er / 100)) + ($statutory_rate_pf * ($pf_esi_rates->epf_ac_2_er / 100)));										
						 }
                        } else {
						$statutory_rate_pf = 15000;						
                         $model->employer_pf_contribution = round(($statutory_rate_pf * ($pf_esi_rates->epf_ac_1_er / 100)) + ($statutory_rate_pf * ($pf_esi_rates->epf_ac_10_er / 100)) + ($statutory_rate_pf * ($pf_esi_rates->epf_ac_2_er / 100)));
   ...
   public function actionStatutoryDetails($id) {
      ...
      if ($model->load(Yii::$app->request->post())) {
        ...
        $ctc = $modelRemu->ctc - $modelRemu->employer_pf_contribution;
				  ...
				  if ($modelRemu->restrict_pf == 'Yes') {
					if($modelRemu->gross_salary <= 15000){
						$statutory_rate_pf = $modelRemu->gross_salary - $modelRemu->hra;
						 ...
						 $modelRemu->employer_pf_contribution = round(($statutory_rate_pf * ($pf_esi_rates->epf_ac_1_er / 100)) + ($statutory_rate_pf * ($pf_esi_rates->epf_ac_10_er / 100)) + ($statutory_rate_pf * ($pf_esi_rates->epf_ac_2_er / 100)));										
						 }
                        } else {
						$statutory_rate_pf = 15000;						
                         $modelRemu->employer_pf_contribution = round(($statutory_rate_pf * ($pf_esi_rates->epf_ac_1_er / 100)) + ($statutory_rate_pf * ($pf_esi_rates->epf_ac_10_er / 100)) + ($statutory_rate_pf * ($pf_esi_rates->epf_ac_2_er / 100)));
		 ...
		 $modelRemu->ctc = $ctc + $modelRemu->employer_pf_contribution;
		 $modelRemu->save(false);
		  $model->save(false);		
...
//$model->type=$emp->etype;
         ...
         $Designation = Designation::find()
                 ...
                 ->one();
        // VEPL/HR/2018-19/APP/E/' . $model->app_no . '&nbsp; &nbsp;  // old  
		...
		<p>&nbsp;</p>
		<p>&nbsp;</p>
		<p><strong><em>Ref: VEPL/HR/2018-19/APP/' . $emp->empcode . '&nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; Date: ' . Yii::$app->formatter->asDate($emp->doj, "dd-MM-yyyy") . '</em></strong></p>

		<p>&nbsp;</p>

		<p><strong>' . $emp->empname . ',</strong><br />';
		if($familymodel)
			$letter .= '<strong> S/o Mr.' . $familymodel->name . ',</strong><br />';		
         if ($empadd->addfield1 != '')
		...
		<p>We are pleased to appoint you in the position of<strong> &ldquo;' . $Designation->designation . '&rdquo;</strong> under the following terms and conditions.</p>
			...
			<li><strong>EMPLOYEE NUMBER</strong><strong>:' . $emp->empcode . '</strong> &amp; <strong>Work Level</strong> : <strong>' . $remunerationmodel->work_level .' '.$remunerationmodel->grade. '</strong></li>
			<li><strong>SALARY &amp; ALLOWANCE</strong><strong>:</strong> You shall be entitled to an annual gross pay of <strong>Rs.' . $remunerationmodel->gross_salary * 12 . '/- </strong>(' . $emp->getIndianCurrency($remunerationmodel->gross_salary * 12) . 'Only) Payable per annum as per Annexure.</li>';
				
				if(!empty($emp->probation)){
					if ($emp->joining_status == 'Experience'){
					$letter .='<li><strong>PROGRESSION:</strong> You shall work under the terms of this letter for a period of <strong>'.$emp->probation.'</strong>. Further Progression will be subjected to your satisfactory performance.</li>';
					} else {
					 $letter .='<li><strong>PROBATION</strong><strong>:</strong> You will be on probation for a period of <strong>'.$emp->probation.'</strong>. Your Confirmation will be subjected to satisfactory performance during the probation period.</li>';			
					}				
				}
				
				
			$letter .='<li><strong>FULL TIME EMPLOYMENT:</strong> This is a full time employment, and therefore you shall devote full time to the work of the company and will not undertake any direct / indirect business or work, honorary or remuneratory, except with prior written permission of the management, in each case.</li>
			...
			<li><strong>LEAVE</strong><strong>:</strong> On Completion of your probation (if any), you will be entitled to earned leave and casual leave as per the rules operational in the company and which is subjected to change.</li>
			<li><strong>COMMUNICATION ADDRESSES:</strong> Your mailing addresses as given in the application will be deemed to be correct for the purpose of sending any communication to you. In case of any change in the same, you will inform the management in writing.</li>';			
			
			if ($emp->category == 'HO Staff' || $emp->category == 'BO Staff' ){
			$letter .='<li><strong>BENEFITS:</strong> Your official tour travel expenses shall be met by the company subjected to the work level eligibility. 
			...
			<li><strong>BUSINESS LOSS COMPENSATION:</strong> You are liable to work minimum 5 years in our organization. In case of your resignation before completing 3 years of service (from the date of joining), you will have to pay a sum of Rs.1,00,000/-(Rupees One Lakh) towards business loss, On-the-Job training cost and administrative expenses. You shall not avail Experience Certificate, Conduct Certificate etc.</li>';			
			 }			
			$letter .='<li>
