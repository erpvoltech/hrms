...
use app\models\VgInsuranceEquipmentSearch;
...
/**
 * VgInsuranceEquipmentController implements the CRUD actions for VgInsuranceEquipment model.
 */
class VgInsuranceEquipmentController extends Controller
{
    /**
     * {@inheritdoc}
     */
    ...
    /**
     * Lists all VgInsuranceEquipment models.
     * @return mixed
     */
    public function actionIndex()
    {
        $searchModel = new VgInsuranceEquipmentSearch();
    ...
    /**
     * Displays a single VgInsuranceEquipment model.
     * @param integer $id
     * @return mixed
    ...
    /**
     * Creates a new VgInsuranceEquipment model.
    ...
    {
        $model = new VgInsuranceEquipment();
    ...
    /**
     * Updates an existing VgInsuranceEquipment model.
    ...
    public function actionPolicyremainderequipment(){
        $propertyRemainder = VgInsuranceEquipment::find()->where('valid_to BETWEEN NOW() AND DATE_ADD(NOW(), INTERVAL 30 DAY)')->all();
        return $this->render('policyremainderequipment', ['propertyRemainder' => $propertyRemainder,]);
    }
    ...
    public function actionEquipmentindexnew() {
        return $this->render('equipmentindexnew');
    }

    /**
     * Deletes an existing VgInsuranceEquipment model.
    ...
    /**
     * Finds the VgInsuranceEquipment model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return VgInsuranceEquipment the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id)
    {
        if (($model = VgInsuranceEquipment::findOne($id)) !== null) {
            return $model;
        }
    ...
    public function actionExportequipment() {
        return $this->render('exportequipment');
    }
    
    public function actionExportequipmentdata() {
        return $this->render('exportequipmentdata');
    }
    
    public function actionImportequipment() {
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            if ($model->uploadtype == 1 && $model->uploaddata == 1) {
                ...
                try {
                    ...
                    foreach ($data as $key => $excelrow) {                         
                        if (!empty($excelrow['Property Type'])) {
                            $modelProperty = new VgInsuranceEquipment();
                ...
                try {
                    foreach ($data as $key => $excelrow) {
                        if (!empty($excelrow['Property Type'])) {
                            $propertyUpdate = VgInsuranceEquipment::find()->where(['insurance_no' => $excelrow['Insurance No']])->one();
                            if ($propertyUpdate) {
                                $modelProperty = VgInsuranceEquipment::findOne($propertyUpdate->id);
                                $modelProperty->icn_id = $excelrow['Insurance Company Name'];
                                $modelProperty->insurance_agent_id = $excelrow['Agent Name'];
                                $modelProperty->property_type = $excelrow['Property Type'];
                                $modelProperty->insurance_no = $excelrow['Insurance No'];
            ...
            unlink($fileName);
            return $this->redirect('equipmentindexnew');
        }

        return $this->render('importequipment', [
                    'model' => $model,
        ]);
    }
}
