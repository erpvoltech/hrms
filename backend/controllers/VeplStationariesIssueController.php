...
class VeplStationariesIssueController extends Controller {
        ...
        return [
            ...
            /*'access' => [
                ...
                'rules' => [
                    ...
                    ['allow' => true,
                        'actions' => ['index', 'create', 'view', 'upadte', 'delete', 'stockqty', 'stockqtycreate'],
                        'allow' => true,
                        'matchCallback' => function ($rule, $action) {
                    return AuthAssignment::Rights('stationary', 'create');
                },
                    ],
                    ['allow' => true,
                        'actions' => ['index', 'view'],
                        'allow' => true,
                        'matchCallback' => function ($rule, $action) {
                    return AuthAssignment::Rights('stationary', 'view');
                },
                    ],
                    ['allow' => true,
                        'actions' => ['index', 'create', 'view', 'update', 'delete', 'stockqty', 'stockqtycreate'],
                        'allow' => true,
                        'matchCallback' => function ($rule, $action) {
                    return AuthAssignment::Rights('stationary', 'update');
                },
                        'roles' => ['@'],
                    ],
                    ['allow' => true,
                        ...
                        'allow' => true,
                        'matchCallback' => function ($rule, $action) {
                    return AuthAssignment::Rights('stationary', 'delete');
                },
                        'roles' => ['@'],
                    ],
                ],
	...
	public function actionIssueReport() {
        $model = new VeplStationariesIssue();
        if ($model->load(Yii::$app->request->post())) {
            return $this->redirect(['issue-report', 'doifrom' => $model->issue_date, 'doito' => $model->doito,]);
        }
        return $this->render('issue-report', ['model' => $model,]);
    }
                ...
                try {
                    if ($flag = $modelGrn->save(false)) {
                        foreach ($modelsGrnItem as $modelGrnItem) {
                            ...
                            $stock = VeplStationariesStock::find()->where(['item_id' => $modelGrnItem->stationaries_id])->one();
                            ...
                            if ($stock) {
                                ...
                                $stockmodel->item_id = $modelGrnItem->stationaries_id;
    ...
    public function actionUpdate($id) {
        ...
        $existingGrnItemId = VeplStationariesIssueSub::find()->select('stationaries_id')->where(['issue_id' => $id])->asArray()->all();
        $existingGrnItemId = ArrayHelper::getColumn($existingGrnItemId, 'stationaries_id');
