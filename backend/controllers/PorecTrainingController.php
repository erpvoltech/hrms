...
class PorecTrainingController extends Controller
{
    ...
    {		
			...
			#$searchModelnew = $searchModel->search(Yii::$app->request->queryParams);			
			/*$dataProvider = new ActiveDataProvider([
				'query' => $query,
				'pagination' => false,
			]);*/
		...
		echo "<script type='text/javascript'>
			$('document').ready(function(){	
				...
				$('.ecodecheckbox').change(function () {
					
					var ecodeval	=	$(this).val();
					//alert('ecodeval: '+ ecodeval);
					if (this.checked){
						$.post('ajax-getecode',{'ecodeval':ecodeval},function(datares){
							//$('#ecode_grid  input[type=checkbox]').prop('checked', false);	
							$('#ecode_display').show();
							$('#ecode_display').append(datares);									
						})
					}else{
						$('#ecodech_'+ecodeval).remove();
					}
					...
					if(count($recruitment_arr[$i]['rec_id']) > 0){						
						for($j=0;$j<count($recruitment_arr[$i]['rec_id']);$j++){
							...
							if($recruitment_arr[$i]['rec_id'][$j] != ''){
								...
								if($recquery_rec->training_status == NULL){
								echo '<tr>
	...
	public function actionAjaxGetofferlettercontent(){
		...
		/*$pomodel->offerletter	= "<p>&nbsp;</p>
						...
						<p><strong>Sub: Offer letter for employment</strong></p>
						<p><strong>&nbsp;</strong></p>
						<p>With reference to your application for a career in our organization and subsequent to the discussions we had, we are pleased to offer you employment in our Organization.</p>
						<p>&nbsp;</p>
						<p>We look forward to you joining us and expect your efforts to play a key in achieving VEPL's goals. We are confident that we will be able to present you with an exciting and challenging career with commensurate rewards.</p>
						<p>&nbsp;</p>
						<p>Your Annual Cost to the Company <strong>(CTC) </strong>will be as discussed and agreed by you. Your compensation is a confidential matter between you and the company and you are expected to maintain secrecy of the same. The breakup of the salary will be issued to you at the time of joining.</p>
						...
						<p>&nbsp;</p>
						<ol>
						<li>1) Original Degree Certificate (For Passed outs) / Provisional & Consolidated Mark List (For those who haven’t received your degree)</li>
						<li>2) Softcopy of all certificates (10th, 12th/Diploma, Degree, provisional certificate, consolidated marksheet)</li>
						<li>3) Softcopy of Identity proofs (Pan Card, Aadhar Card, Voter ID, Driving License, Passport)</li>
						<li>4) Hardcopy of Agreement (Use only Rs. 20/- sheet, extra sheets in green colour legal sheet only) with witness and guarantors signature.</li>
						<li>5) Hard copy of Police Verification / Clearance Certificate</li>
						<li>6) Soft copy of Family Photo (Visiting Card Size). </li>
						<li>7) Softcopy of Bank passbook or Cheque Leaf.</li>
						<li>8) Softcopy of your photo in White Background with Blazer & Tie</li>
						<li>9) Medical Certificate with the required test reports to be submitted.</li>	
						<li>10) Safety Shoe (EN-345, IS:15298)</li>
						<li>11) Soft copy of Training Certificate</li>
						<li>12) Soft copy of Parents Aadhaar </li>
						</ol>
						<ol start='10'>
						<li>Original certificates of the above to be presented for</li>
						</ol>
						...
						<p>On submission of the above-mentioned documents and subsequent to you joining us, your appointment letter will be issued.</p>
						<p>&nbsp;</p>
						<p> Company is not responsible for any offers made orally. It is valid only if given in writing. The Offer Letter is enclosed in duplicate</p>
						<p>&nbsp;</p>
						<p>Please follow the instructions as mentioned in the mail. For <strong>VOLTECH Engineers Private Limited</strong></p>
						<p><strong>&nbsp;</strong></p>
			...
			$autogen_offerno	= trim('0001');
				...
				/*$model->body = "<p align='center'><strong>OFFER LETTER</strong></p>
					...
					$model->body ="<p>&nbsp;</p>
						...
						$mailmodel->cc = "pr@voltechgroup.com";
	...
	public function actionAjaxGetecode(){
		$data = Yii::$app->request->post();
		$ecode_id	=	$data['ecodeval'];
		$empmodel	=	EmpDetails::findOne($ecode_id);
		/*echo "<div id='ecodech_$ecode_id'>";		
		echo "<span>".$empmodel->empcode."</span>";
		echo "<span>     </span>";
		echo "<span>".$empmodel->empname."</span>";
		echo "</div>";		*/
		
		echo "<tr id='ecodech_$ecode_id'>";		
		echo "<td>".$empmodel->empcode."</td>";		
		echo "<td>".$empmodel->empname."</td>";
		echo "</tr>";	
	}
